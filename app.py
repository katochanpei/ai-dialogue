...
def _bootstrap_secrets() -> None:
    ...
    try:
        for key in ("GEMINI_API_KEY", "APP_PASSWORD", "MULTI_USER_MODE"):
...
### 💰 コストと制限
- **完全無料**（Geminiの無料枠を使用）
- 全員合計で**1日1500リクエスト**が上限（Gemini 2.0 Flash）
- 1議論あたり 10〜20リクエスト消費
- 上限超えたらその日は使えへん（翌朝復活）
...
- 終わったらログが `logs/` に保存される
    ...
    with cols[1]:
        st.metric("モデル", "Gemini 2.0 Flash")
    with cols[2]:
